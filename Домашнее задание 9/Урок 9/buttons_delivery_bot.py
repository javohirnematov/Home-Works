...
def num_button():
    ...
    #Создаем сами кнопки
    num = types.KeyboardButton('Отправить номер/Send a number', request_contact=True)
...
def loc_button():
    ...
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    # Создаем сами кнопки
    loc = types.KeyboardButton('Отправить геопозицию/Send a geo location', request_location=True)
    # Добавляем кнопки в пространство
    kb.add(loc)
    return kb
...
def main_menu_buttons(products_from_db):
    ...
    cart = types.InlineKeyboardButton(text='Корзина/Cart', callback_data='cart')
...
def choose_product_count(amount=1, plus_or_minus=''):
    ...
    #Создаем сами кнопки
    back = types.InlineKeyboardButton(text='Назад/Back', callback_data='back')
    plus = types.InlineKeyboardButton(text='+', callback_data='increment')
    minus = types.InlineKeyboardButton(text='-', callback_data='decrement')
    count = types.InlineKeyboardButton(text=str(amount), callback_data=str(amount))
    add_to_cart = types.InlineKeyboardButton(text='Добавить в корзину/Add to cart', callback_data='to_cart')
...
def cart_buttons():
    ...
    #Создаем сами кнопки
    order = types.InlineKeyboardButton(text='Оформить заказ/Place an order', callback_data='order')
    clear = types.InlineKeyboardButton(text='Очистить корзину/Clear cart', callback_data='clear')
    back = types.InlineKeyboardButton(text='Назад/Back', callback_data='back')
    #Добавляем кнопки в пространство
