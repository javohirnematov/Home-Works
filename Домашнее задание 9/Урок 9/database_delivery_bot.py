...
connection = sqlite3.connect('database_delivery_bot.db', check_same_thread=False)
# Связь SQL с Python
...
def register(id, name, num, loc):
    sql.execute('INSERT INTO users VALUES(?, ?, ?, ?);', (id, name, num, loc))          # sql.execute добавляем в БД
    #Фиксируем изменения
    connection.commit()
...
#Вывод id продуктов
def get_pr_name_id():
    products = sql.execute('SELECT pr_id, pr_name, pr_amount, pr_price FROM products;').fetchall()
    return products

#Получить имя продукта
def get_pr_name(pr_id):
    product = sql.execute('SELECT pr_name FROM products WHERE pr_id=?', (pr_id,))
    return product.fetchone()

#Получить id продукта если в БД его количестао >0
def get_pr_id():
    prods = sql.execute('SELECT pr_id, pr_name, pr_amount FROM products;').fetchall()
...
def del_cart(user_id):
    ...
    sql.execute(f'UPDATE products SET pr_amount={amount + pr_quantity} WHERE pr_name=?;', (pr_name[0],))
    # Фиксируем изменения
    connection.commit()
...
# sql.execute('INSERT INTO products (pr_name, pr_amount, pr_price, pr_des, pr_photo) VALUES ("Лаваш/Lavash", 5000, 27990, "Вкусный, сочный", "https://i.postimg.cc/KjMbw0hM/da9581ee81134ea59740836aaf9e4a57-e1580887902436.jpg");')
# connection.commit()